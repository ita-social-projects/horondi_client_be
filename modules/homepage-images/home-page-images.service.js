...
class HomePageImagesService {
  ...
  addHomePageLooksImage(data) {
    return this.uploadImages([data.images]).then((resizedImage) => {
      if (!resizedImage) { throw new RuleError(IMAGES_WERE_NOT_CONVERTED, BAD_REQUEST); }
  ...
  async uploadImages(data) {
    ...
    if (!resizedImages) { throw new RuleError(IMAGES_WERE_NOT_CONVERTED, BAD_REQUEST); }

    return resizedImages;
  }

  saveUpdatedLooksImages(id, imageToUpload) {
    return this.uploadImages([imageToUpload]).then((images) => LooksImages.findByIdAndUpdate(
      id,
      { images: images[0] },
      {
        new: true,
  ...
  async deleteImages(imagesToDelete) {
    ...
    if (!deletedImages) { throw new RuleError(IMAGES_WERE_NOT_CONVERTED, BAD_REQUEST); }
