...
class OrdersService {
    ...
    async getAllOrders({skip, limit, filter = {},sort}) {
        ...
        if (search) {
            ...
            filterObject.$or = [
                {'user.firstName': {$regex: `${firstParam || secondParam}`, $options: 'i'}},
                {'user.lastName': {$regex: `${firstParam || secondParam}`, $options: 'i'}},
                {orderNumber: {$regex: `${firstParam || secondParam}`, $options: 'i'}},
            ];
        }
        ...
        const count = await Order.find(filterObject).countDocuments();
        return {
            items,
            count,
        };
