...
const cronRecalculateAdditionalPrice = async (model) => {
  ...
  documents.map(async (document) => {
    document.additionalPrice = document.additionalPrice[1].type === ABSOLUTE_INDICATOR
...
const cronRecalculateBasePrice = async (model) => {
  const documents = await model.find();
  documents.map(async (document) => {
    document.basePrice = document.basePrice[1]
...
const cronRecalculatePocketBack = async (model) => {
  const documents = await model.find();
  documents.map(async (document) => {
    document.additionalPrice = document.additionalPrice[1]
