...
const news = [
    {
        lang: 'uk',
        title: 'Модні новинки сумок на сезон осінь-зима 2019-2020',
        text: fs.readFileSync(path.join(articlePath, 'article1UK.html'), 'utf8').split('\n').join(''),
        images: {
    ...
    {
        lang: 'en',
        title: 'Fasionable new bags for fall-winter 2019-2020 season',
        text: fs.readFileSync(path.join(articlePath, 'article1EN.html'), 'utf8').split('\n').join(''),
        images: {
            primary: { medium: 'https://ukr.media/static/ba/aimg/3/9/3/393626_1.jpg' },
            additional: [
                { medium: 'https://ukr.media/static/ba/aimg/3/9/3/393626_3.jpg' },
    ...
    {
        lang: 'uk',
        title: 'Що носити модницям влітку 2020: добірка трендових сумок для спекотного сезону',
        text: fs.readFileSync(path.join(articlePath, 'article2UK.html'), 'utf8').split('\n').join(''),
        images: {
    ...
    {
        lang: 'en',
        title: 'A selection of trendy bags for the hot season of summer 2020',
        text: fs.readFileSync(path.join(articlePath, 'article2EN.html'), 'utf8').split('\n').join(''),
        images: {
            primary: { medium: 'https://s.032.ua/section/newsInternalIcon/upload/images/news/icon/000/052/185/trendi-lita-golovna_5edf9266621db.jpg' },
            additional: [
                { medium: 'https://s.032.ua/section/newsInText/upload/images/news/intext/000/052/185/solomani_5edf8e4b209ed.jpg' },
    ...
    {
        lang: 'uk',
        title: 'Аксесуар на пояс, зручна сумка, стильна штучка!',
        text: fs.readFileSync(path.join(articlePath, 'article3UK.html'), 'utf8').split('\n').join(''),
        images: {
    ...
    {
        lang: 'en',
        title: 'Belt accessory, comfortable bag, stylish thingy!',
        text: fs.readFileSync(path.join(articlePath, 'article3EN.html'), 'utf8').split('\n').join(''),
        images: {
            primary: { medium: 'https://images.stylight.net/image/upload/t_web_post_500x667/q_auto,f_auto/post-c27c63a5b58deb0d4535ddd5993e68b2e4d0776d756fdc41c0f7c391.jpg' },
            additional: [
                { small: 'https://intersumka.ua/image/catalog/_all/banan2.jpg' },
    ...
    {
        lang: 'uk',
        title: 'Даруємо знижку - 10%',
        text: fs.readFileSync(path.join(articlePath, 'article4UK.html'), 'utf8').split('\n').join(''),
        images: {
    ...
    {
        lang: 'en',
        title: 'Delivery to the EU now available',
        text: fs.readFileSync(path.join(articlePath, 'article5EN.html'), 'utf8').split('\n').join(''),
        images: {
